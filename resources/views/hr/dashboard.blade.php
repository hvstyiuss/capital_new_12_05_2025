...
<div class="container-fluid py-4">
    ...
    <div class="alert alert-info border-info alert-dismissible show mb-4" role="alert" id="pendingDemandesAlert" data-no-auto-hide="true" data-alert-key="pending-demandes-alert">
        <div class="d-flex align-items-center">
            ...
            <div class="flex-grow-1">
                <h6 class="alert-heading mb-1">Demandes en attente</h6>
                <p class="mb-0">Vous avez <strong>{{ $pendingDemandesCount }}</strong> demande(s) de congé en attente de traitement</p>
            </div>
            <a href="{{ route('hr.leaves.agents', ['statut' => 'pending']) }}" class="btn btn-info btn-sm me-2">
        ...
        <div class="mt-3">
            @foreach($pendingDemandesForChef as $demande)
                @php
                    ...
                    $dateDebut = $avisDepart && $avisDepart->date_depart ? \Carbon\Carbon::parse($avisDepart->date_depart)->format('d/m/Y') : 'N/A';
                    ...
                    <div class="d-flex justify-content-between align-items-center">
                        <div>
                            ...
                            <small class="text-muted d-block">
                                <i class="fas fa-calendar me-1"></i>Départ: {{ $dateDebut }} | 
                                <i class="fas fa-clock me-1"></i>{{ $nbrJours }} jour(s)
                            </small>
                        </div>
                        <a href="{{ route('hr.leaves.agents', ['statut' => 'pending']) }}" class="btn btn-sm btn-outline-info">
        ...
        <div class="mt-3">
            @foreach($pendingMutationsForChef as $mutation)
                @php
                    ...
                    $agentPpr = $mutation->user ? $mutation->user->ppr : 'N/A';
                    $toEntiteName = $mutation->toEntite ? $mutation->toEntite->name : 'N/A';
                    $dateDepot = $mutation->created_at ? \Carbon\Carbon::parse($mutation->created_at)->format('d/m/Y') : 'N/A';
                    $mutationType = ucfirst($mutation->mutation_type);
                @endphp
                <div class="bg-white rounded p-3 mb-2">
                    <div class="d-flex justify-content-between align-items-center">
                        <div>
                            <strong>{{ $agentName }}</strong> <small class="text-muted">({{ $agentPpr }})</small>
                            <small class="text-muted d-block">
                                <i class="fas fa-building me-1"></i>Destination: {{ $toEntiteName }} | 
                                <i class="fas fa-calendar me-1"></i>Date de dépôt: {{ $dateDepot }} | 
                                <i class="fas fa-tag me-1"></i>Type: {{ $mutationType }}
                            </small>
                        </div>
                        <a href="{{ route('mutations.agent-requests', ['status' => 'pending']) }}" class="btn btn-sm btn-outline-info">
                            <i class="fas fa-eye me-1"></i>Voir
                        </a>
                    </div>
                </div>
    ...
    <div class="alert alert-info border-info mb-4" role="alert">
        <div class="d-flex align-items-center">
            ...
            <div class="flex-grow-1">
                <h6 class="alert-heading mb-1">Déclaration de retour</h6>
                <p class="mb-0">Aujourd'hui est votre date de retour - Veuillez déclarer votre retour</p>
                @php
            ...
            <a href="{{ route('hr.leaves.declare-retour') }}" class="btn btn-info btn-sm">
                <i class="fas fa-edit me-1"></i>Déclarer le retour
            </a>
        </div>
    </div>
    @endif
    ...
    @if(isset($recentMutationChanges) && count($recentMutationChanges) > 0)
        @foreach($recentMutationChanges as $mutationChange)
            @php
                $mutation = $mutationChange['mutation'];
                $status = $mutationChange['status'];
                $isRecent = $mutationChange['is_recent'];
                $toEntiteName = $mutation->toEntite ? $mutation->toEntite->name : 'N/A';
                $dateDepot = $mutation->created_at ? \Carbon\Carbon::parse($mutation->created_at)->format('d/m/Y') : 'N/A';
                ...
                $approverName = null;
                if ($status == 'approved') {
                    // For approved mutations, show super Collaborateur Rh who validated
                    ...
                    if ($mutation->rejectedBySuperRh) {
                        $approverName = $mutation->rejectedBySuperRh->fname . ' ' . $mutation->rejectedBySuperRh->lname;
                    } elseif ($mutation->rejectedByCurrentDirection) {
                        $approverName = $mutation->rejectedByCurrentDirection->fname . ' ' . $mutation->rejectedByCurrentDirection->lname;
                    } elseif ($mutation->rejectedByDestinationDirection) {
                        $approverName = $mutation->rejectedByDestinationDirection->fname . ' ' . $mutation->rejectedByDestinationDirection->lname;
                    }
                }
            @endphp
            @if($status == 'approved')
                    ...
                    <div class="flex-grow-1">
                        ...
                        <div class="small">
                            ...
                            <div class="mt-2 p-2 bg-light rounded">
                                ...
                                <span class="text-success fw-bold">{{ \Carbon\Carbon::parse($mutation->date_debut_affectation)->format('d/m/Y') }}</span>
                                <br>
                ...
                <div class="d-flex align-items-center">
                    ...
                    <div class="flex-grow-1">
                        ...
                        <div class="small">
                            ...
                            <div><i class="fas fa-tag me-1"></i><strong>Type:</strong> {{ ucfirst($mutation->mutation_type) }}</div>
                            @if($approverName)
                            <div><i class="fas fa-user-times me-1"></i><strong>Rejetée par:</strong> {{ $approverName }}</div>
                            @endif
                        </div>
                    </div>
    ...
    @if(isset($pendingSuperCollaborateurRhMutations) && count($pendingSuperCollaborateurRhMutations) > 0)
        @foreach($pendingSuperCollaborateurRhMutations as $mutation)
            @php
                $toEntiteName = $mutation->toEntite ? $mutation->toEntite->name : 'N/A';
                $dateDepot = $mutation->created_at ? \Carbon\Carbon::parse($mutation->created_at)->format('d/m/Y') : 'N/A';
                $userName = $mutation->user ? ($mutation->user->fname . ' ' . $mutation->user->lname) : 'N/A';
            @endphp
            @php
                // Determine the stage
    ...
    <div class="card border-0 shadow-sm mb-4 overflow-hidden">
        ...
        <div class="card-body p-4">
            <div class="row g-4">
                ...
                <div class="col-lg-9">
                    @php
                        // Use data prepared by the controller / services
                    ...
                    <div class="mb-4">
                        ...
                        @php
                            ...
                            // Check if entity should show direction
                            // Only show direction for entities under regional directions
                            // Hide for entities under central administration (Directeur Général, Secrétaire Général, etc.)
                                ...
                                <div class="info-card h-100">
                                    <div class="d-flex align-items-start">
                                        ...
                                        <div class="flex-grow-1 ms-3">
                                            ...
                                            @php
                                                // Traverse up the parent chain to find lieu_affectation
                                                ...
                                                if ($currentEntite) {
                                                    ...
                                                    if ($villeAffectation === 'Non définie' && isset($direction) && $direction && $direction->lieu_direction) {
                                                        $villeAffectation = $direction->lieu_direction;
                                                    }
                                                }
                                            @endphp
