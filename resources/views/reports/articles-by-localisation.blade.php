...
@section('title', 'Articles par Localisation')
...
<div class="content-card">
    ...
    <div class="header-content">
        <div>
            <h1 class="card-title">Articles par Localisation</h1>
            <p class="card-subtitle">Analysez les articles par localisation géographique avec des statistiques détaillées</p>
        </div>
    ...
    <div class="filters-form">
        <form method="GET" action="{{ route('reports.articles-by-localisation') }}" class="filters-grid">
            ...
            <div class="form-actions">
                ...
                <a href="{{ route('reports.articles-by-localisation') }}" class="btn btn-outline-secondary">
    ...
    <div class="bg-white/80 backdrop-blur-xl rounded-3xl shadow-2xl p-8 border border-white/20 mb-8">
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
            <div>
                ...
                <canvas id="chartVendusInvendusLocalisation"></canvas>
            </div>
            <div>
                <h3 class="text-lg font-bold text-gray-800 mb-3">Revenus (DH)</h3>
                <canvas id="chartRevenusLocalisation"></canvas>
            </div>
        </div>
    </div>
    @push('scripts')
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        const c1 = document.getElementById('chartVendusInvendusLocalisation');
        if (c1 && window.Chart) {
        ...
        const c2 = document.getElementById('chartRevenusLocalisation');
        if (c2 && window.Chart) {
    ...
    @php
        $headers = ['ID', 'Numéro', 'Date', 'Localisation', 'Forêts', 'Essences', 'Exploitant', 'Statut', 'Prix de Vente', 'Actions'];
        $rows = $articles->map(function ($article) {
            $date = optional($article->date_adjudication)->format('d/m/Y') ?: 'N/A';
            $localisation = method_exists($article, 'localisations') && $article->localisations && $article->localisations->count()
                ? $article->localisations->map(fn($l) => e($l->CODE.' - '.($l->ENTITE ?? '')))->implode(', ')
                : 'N/A';
