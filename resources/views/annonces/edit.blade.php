...
<div class="container mx-auto px-4 py-8">
        ...
        <div class="bg-white/80 dark:bg-gray-800/80 backdrop-blur-xl rounded-3xl shadow-2xl border border-white/20 dark:border-gray-700/20 p-8">
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <h1 class="text-4xl font-bold bg-gradient-to-r from-blue-600 to-cyan-600 bg-clip-text text-transparent">Modifier une Annonce</h1>
                        <p class="text-gray-600 dark:text-gray-400 text-lg mt-2">Modifiez les informations de l'annonce</p>
                    </div>
                </div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700 p-8">
        <form action="{{ route('annonces.update', $annonce) }}" method="POST" enctype="multipart/form-data" id="annonceForm" novalidate autocomplete="off">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <select 
                        ...
                        @foreach($users as $user)
                            <option value="{{ $user->ppr }}" {{ old('ppr', $annonce->ppr) == $user->ppr ? 'selected' : '' }}>
                                {{ $user->ppr }} - {{ $user->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <textarea 
                        ...
                        placeholder="Saisissez le contenu de l'annonce...">{{ old('content', $annonce->content) }}</textarea>
                    @error('content')
                        <p class="text-red-500 text-sm mt-1 validation-error">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <select 
                        ...
                        @foreach($typesAnnonces as $type)
                            <option value="{{ $type->id }}" {{ old('type_annonce_id', $annonce->type_annonce_id) == $type->id ? 'selected' : '' }}>
                                {{ $type->nom }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select 
                        ...
                        @foreach($entites as $entite)
                            <option value="{{ $entite->id }}" {{ in_array($entite->id, old('entites', $annonce->entites->pluck('id')->toArray())) ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <div class="mb-4">
                        <img src="{{ Storage::url($annonce->image) }}" alt="Image actuelle" class="w-48 h-48 object-cover rounded-lg border border-gray-300">
                    </div>
                </div>
                @endif
                ...
                <div>
                    <label for="image" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                        <i class="fas fa-image text-blue-500 mr-2"></i>
                        {{ $annonce->image ? 'Remplacer l\'image' : 'Image (optionnel)' }}
                    </label>
                    <input 
                        type="file" 
                        id="image" 
